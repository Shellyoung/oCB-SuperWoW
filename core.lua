...
local IgnoredSpells = {
	' DND', 'DND)', ' TEST', 'TEST)', 'Quest', 'Passive', 'Effect', '^Chilled$', '^Dazed$', 'Proc$', 'Charge Stun', 'Kel\'Thuzad Channel', 'Visual$'
}

local function IgnoreSpell(Name)
	if not Name then
		return true
	end
	...
	for _, Pattern in ipairs(IgnoredSpells) do
		if string.find(S, string.lower(Pattern)) then
			return true
		end
	end
end

local function IgnoreID(I)
	return	I == 51697
			or I == 52657 -- Felstone heal
end
...
function oCB:CastEvent(casterGUID, targetGUID, eventType, spellID, castDuration)
	...
	local Name, _, _, Min, Max = SpellInfo(spellID)
    
    if eventType == 'START' or eventType == 'CHANNEL' or casterGUID == 'TargetBar' then
		if IgnoreSpell(Name) or IgnoreID(spellID) then
			return
		end
    ...
    elseif eventType == 'CAST' or eventType == 'FAIL' then
		...
		if not ((eventType == 'CAST' and Bar.eventType == 'CAST' and GetTime() - (Bar.StartTime or 0) < 0.020)
					or IgnoreSpell(Name) or IgnoreID(spellID)
					or (Bar:IsVisible() and (Bar.StartTime or 0) + castDuration > GetTime())
					or (Min == 0 and Max == 103 and castDuration == 0)) then
			oCB:TargetCastStop(casterGUID, targetGUID, eventType, spellID)
		elseif (casterGUID == targetGUID and Bar.Source and Bar.Source == Bar.Target and GetTime() - (Bar.StartTime or 0) < 0.020) then
			oCB:HideTargetBar()
		end
	elseif eventType == 'MAINHAND' or eventType == 'OFFHAND' then
		if GetTime() - (Bar.StartTime or 0) < 0.020 then
			oCB:HideTargetBar()
		end
	end
end
